...
import { FaCheckCircle, FaExclamationCircle, FaTimesCircle, FaInfoCircle, FaSearch, FaLightbulb, FaDownload } from 'react-icons/fa';
...
const AnalysisResults = () => {
    ...
    return (
        <Box bg="transparent" minH="calc(100vh - 64px)">
            ...
            <Box className="screen-only">
                <Container maxW="container.xl" pt={12}>
                    ...
                    <Flex justify="center" gap={4} flexWrap="wrap">
                        ...
                        <CustomButton variant="outline" onClick={() => window.print()}>
            ...
            <Box className="print-only" display="none" p={8} bg="white" color="black">
                <VStack align="stretch" spacing={6}>
                    {/* Header */}
                    <Flex justify="space-between" align="center" borderBottom="2px solid #E2E8F0" pb={4}>
                        <VStack align="start" spacing={1}>
                            <Heading size="lg" color="gray.800">ResumeAI Analysis Report</Heading>
                            <Text fontSize="sm" color="gray.500">Generated on {new Date().toLocaleDateString()}</Text>
                        </VStack>
                        <Box textAlign="right">
                            <Text fontSize="3xl" fontWeight="bold" color="#7AAACE">{result.overallScore}%</Text>
                            <Text fontSize="xs" color="gray.500" fontWeight="medium">Match Score</Text>
                        </Box>
                    </Flex>

                    {/* Summary section */}
                    <Box borderBottom="1px solid #E2E8F0" pb={4}>
                        <Heading size="sm" mb={2} color="gray.700">Analysis Summary</Heading>
                        <Text fontSize="sm" color="gray.600" lineHeight="relaxed">
                            {result.overallScore >= 70
                                ? 'Good match — your resume has strong alignment with the job description with room to improve.'
                                : 'Moderate match — your resume requires updates and additions to better match the target job description.'}
                        </Text>
                    </Box>

                    {/* Matched Elements */}
                    <Box borderBottom="1px solid #E2E8F0" pb={4} className="print-avoid-break">
                        <Heading size="sm" mb={3} color="green.700">✅ Matched Elements</Heading>
                        {matchedElements.skills.length > 0 && (
                            <Box mb={3}>
                                <Text fontWeight="bold" fontSize="xs" color="gray.500" mb={1}>Matched Skills</Text>
                                <Text fontSize="sm" color="gray.700">{matchedElements.skills.join(', ')}</Text>
                            </Box>
                        )}
                        {matchedElements.keywords.length > 0 && (
                            <Box mb={3}>
                                <Text fontWeight="bold" fontSize="xs" color="gray.500" mb={1}>Matched Keywords</Text>
                                <Text fontSize="sm" color="gray.700">{matchedElements.keywords.join(', ')}</Text>
                            </Box>
                        )}
                        {matchedElements.experience.length > 0 && (
                            <Box>
                                <Text fontWeight="bold" fontSize="xs" color="gray.500" mb={1}>Matched Experience</Text>
                                <VStack align="stretch" spacing={1}>
                                    {matchedElements.experience.map((exp, i) => (
                                        <Text key={i} fontSize="sm" color="gray.700">• {exp}</Text>
                                    ))}
                                </VStack>
                            </Box>
                        )}
                    ...
                    {/* Missing Elements */}
                    <Box borderBottom="1px solid #E2E8F0" pb={4} className="print-avoid-break">
                        <Heading size="sm" mb={3} color="red.700">❌ Missing Elements (Gaps)</Heading>
                        {missingElements.skills.length > 0 && (
                            <Box mb={3}>
                                <Text fontWeight="bold" fontSize="xs" color="gray.500" mb={1}>Missing Skills</Text>
                                <Text fontSize="sm" color="gray.700">{missingElements.skills.join(', ')}</Text>
                            </Box>
                        )}
                        {missingElements.keywords.length > 0 && (
                            <Box mb={3}>
                                <Text fontWeight="bold" fontSize="xs" color="gray.500" mb={1}>Missing Keywords</Text>
                                <Text fontSize="sm" color="gray.700">{missingElements.keywords.join(', ')}</Text>
                            </Box>
                        )}
                        {missingElements.experience.length > 0 && (
                            <Box>
                                <Text fontWeight="bold" fontSize="xs" color="gray.500" mb={1}>Experience Gaps</Text>
                                <VStack align="stretch" spacing={1}>
                                    {missingElements.experience.map((exp, i) => (
                                        <Text key={i} fontSize="sm" color="gray.700">• {exp}</Text>
                                    ))}
                                </VStack>
                            </Box>
                    ...
                    {keywords.length > 0 && (
                        <Box borderBottom="1px solid #E2E8F0" pb={4} className="print-avoid-break">
                            <Heading size="sm" mb={3} color="orange.700">💡 Keyword Suggestions</Heading>
                            <VStack align="stretch" spacing={3}>
                                {keywords.map((kw, i) => (
                                    <Box key={i} p={2} bg="gray.50" borderRadius="sm">
                                        <Flex justify="space-between" align="center" mb={1}>
                                            <Text fontWeight="bold" fontSize="sm" color="gray.800">{kw.keyword}</Text>
                                            <Badge colorScheme={priorityColor(kw.priority)} fontSize="2xs">
                                                {kw.priority.toUpperCase()} - {kw.suggestedSection.toUpperCase()}
                                            </Badge>
                                        </Flex>
                                        {kw.exampleUsage && (
                    ...
                    {/* Recommendations */}
                    {recommendations.length > 0 && (
                        <Box className="print-avoid-break">
                            <Heading size="sm" mb={3} color="blue.700">📋 Recommendations</Heading>
                            <VStack align="stretch" spacing={2}>
                                {recommendations.map((rec, i) => (
                                    <Text key={i} fontSize="sm" color="gray.700">{i + 1}. {rec}</Text>
                                ))}
                            </VStack>
                        </Box>
                    )}
